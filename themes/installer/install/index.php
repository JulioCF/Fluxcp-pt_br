<?php if (!$session->installerAuth): ?>
	<form action="<?php echo $this->url ?>" method="post">
		<p>
			Por favor insira a <em>senha de instalação</em> (setada em application.php) para continuar com a atualização.
		</p>
		<p>
...
<?php else: ?>
	<?php if (isset($permissionError)): ?>
		...
		<p>Uh oh, o instalador encontrou um erro de permissão enquanto tentava executar um dos arquivos SQL!</p>
		<p>Isso tipicamente significa que a query falhou devido a falta de permissão de usuários/databases/tabelas no MySQL.</p>
	...
	<?php else: ?>
		<p class="menu">
			<a href="<?php echo $this->url($params->get('module'), null, array('logout' => 1)) ?>" onclick="return confirm('Você tem certeza que deseja fazer logout?')">Logout</a> |
			<a href="<?php echo $this->url($params->get('module'), null, array('update_all' => 1)) ?>" onclick="return confirm('Ao fazer essa ação, alteração serão feitas em seu banco de dados.\n\Você tem certeza que quer continuar ao instalar o Flux e seus respectivos updates?')"><strong>Instalar ou Atualizar Tudo</strong></a>
		</p>
